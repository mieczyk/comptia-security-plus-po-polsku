...
Chyba najtrafniejszym polskim odpowiednikiem tego terminu, zarówno znaczeniowo jak i funkcjonalnie jest ***śluza bezpieczeństwa***. W dosłownym tłumaczeniu jest to *przedsionek kontroli dostępu*, jednakże w rzeczywistości chyba nikt tak tego nie nazywa. Inną nazwą tego środka bezpieczeństwa, z którą można się jeszcze spotkać, jest *mantrap* (dosłownie: pułapka na ludzi).
...
- Przykładowo: drzwi do kolejnego pomieszczania nie otworzą się dopóki nie zamkniemy drzwi wejściowych i nie uwierzytelnimy się w odpowiedni sposób (np. przykładając kartę dostępu do czytnika).  Procedura przejścia przez taki przedsionek może się różnić w zależności od potrzeb, ale cel jest jednakowy: kontrola dostępu do wybranego obszaru placówki. Takie pomieszczenie kontrolne może być niewielkie, żeby umożliwić przejście tylko jednej osobie na raz.
- Chyba najpopularniejszą formą takiego przedsionka  jest pomieszczenie z parą drzwi. Pierwsze wejście (z zewnątrz) wymaga jakiejś formy autoryzacji (np. zamek elektroniczny otwierany kartą dostępu). Drugie drzwi, prowadzące do chronionej części placówki, otwierają się dopiero, gdy pierwsze się zamkną. Z drugiej strony, nikt nie wejdzie przez pierwsze drzwi, gdy drugie się nie zamkną i osoba znajdująca się w środku nie przejdzie dalej.
- Pozwalają uporządkować *przepływ* ludzi wchodzących i wychodzących, umożliwiają zastosowanie dowolnych środków weryfikacji (karty dostępu, czytniki biometryczne, kody PIN - wtedy upewniamy się, że nikt nie podejrzy kodu za pleców).
- W połączeniu z monitoringiem wideo pozwala na dodatkową weryfikację (np. wykrywanie niebezpiecznych obiektów lub podejrzanego zachowania), a także udokumentowanie wszystkich wizyt. Przydatne gdy ktoś użyje kradzionej karty dostępu - w bazie danych zostanie zarejestrowane wejście właściciela, ale na nagraniu może okazać się, że to była zupełnie inna osoba.
## Fencing
...
## Video surveillance
- Używanie kamer do rejestrowania obrazu oraz alarmowania w razie potrzeby. Jeśli nasz system pozwala na podgląd obrazu w czasie rzeczywistym, to nic innego jak monitoring za pomocą kamer przemysłowych CCTV (*Closed Circuit TeleVision*). Sieć kamer pozwala na obserwację wielu miejsc placówki jednocześnie z jednego stanowiska (np. z pokoju pracowników ochrony).
	- Oprócz podglądu na żywo oraz zapisu materiału wideo na trwałych nośnikach danych, nowoczesne kamery i korzystające z nich systemy posiadają dodatkowe funkcje, takie jak automatyczne wykrywanie ruchu, które od razu zaalarmuje obserwatora (jest to szczególnie istotne, gdy operator musi nadzorować kamery umieszczone w wielu miejscach); rozpoznawanie twarzy, tablic rejestracyjnych i innych obiektów (jak np. pusta paczka Doritos); wbudowane [promienniki podczerwieni](https://solidsecurity.pl/blog/jak-dziala-promiennik-podczerwieni-w-kamerach), które umożliwiają lepsze *widzenie* po zmroku. Nie wspominając o coraz doskonalszych systemach przetwarzania obrazu opartych na rozwiązaniach AI.
- Oprócz podglądu w czasie rzeczywistym, systemy monitoringu zapisują materiał wideo na nośnikach danych, który może posłużyć do późniejszego dochodzenia, jeśli zajdzie taka konieczność (np. policja może wystąpić o udostępnienie nagrań celem zabezpieczenia materiału dowodowego).
- Mnogość dostępnego dziś asortymentu pozwala nam dobrać odpowiedni sprzęt do naszych potrzeb, uwzględniając przy tym możliwości finansowe. Od tanich, czarno-białych kamer o niskiej rozdzielczości, po drogie kamery Ultra HD z dobrymi obiektywami, czujnikami ruchu, wykrywaniem twarzy oraz określonych obiektów - oczywiście z dostępem bezprzewodowym.
- Kamery wykrywające ruch aktywują się i zaczynają nagrywać dopiero po wykryciu ruchu - pozwala to na oszczędność baterii (jeśli kamera jest zasilana akumulatorem) oraz miejsca na nośniku danych. Jest to przydatne w miejscach, które są rzadko uczęszczane, więc każdy ruch powinien wzbudzić naszą czujność.
## Security guard
...
- Pracownicy ochrony są niezbędni, gdy potrzebna jest ludzki osąd bądź interwencja. Można ich często spotkać przy wejściu do budynku (recepcja) bądź przy bramie wjazdowej na teren placówki (portiernia). Stacjonują również w specjalnych pokojach ochrony, gdzie znajduje centrum zarządzania systemami ochrony, włączając w to monitoring. Są także odpowiedzialni za wykonywanie regularnych obchodów.
- Bardzo często pracownicy ochrony odpowiadają również za prowadzenie dziennika odwiedzin (ang. *visitors log*), w którym zapisywane są dane osób odwiedzających wraz z godziną wejścia oraz wyjścia.
- Warto mieć na uwadze, że nie jest to stuprocentowo skuteczny środek zabezpieczający, ponieważ ludzie też są omylni i mogą wydać błędny osąd, szczególnie w sytuacji, gdy atakujący posłuży się wyrafinowaną [socjotechniką](../2-threats-vulnerabilities-mitigations/2-2-threat-vectors-and-attack-surfaces.md#human-vectorssocial-engineering). Trzeba również pamiętać, że **praca człowieka jest relatywnie droga** w porównaniu z technicznymi środkami (włączając w to wdrożenie oraz utrzymanie), jednakże często okazuje się niezbędna. Duże koszta są głównym powodem ograniczenia liczby pracowników ochrony i wspieranie ich automatycznymi rozwiązaniami.
## Access badge
