...
- **Honeyfile** - w odróżnieniu od wspomnianych wcześniej pułapek honeypots/honeynets, których głównym zadaniem jest badanie i analiza zachowań potencjalnego włamywacza, pliki-pułapki służą przede wszystkim do wykrywania włamań (*intrusion detection*).
	- Honeyfile to plik zawierający unikatowe i pozornie atrakcyjne dane, które są później łatwe do namierzenia. Plik jest przeważnie umieszczany w miejscu, do którego zwyczajny użytkownik nie ma dostępu, ale atakujący po uzyskaniu nieautoryzowanego zajrzy tam z dużym prawdopodobieństwem. Jeśli dane z tego pliku zostaną wykryte gdziekolwiek poza tym miejscem (np. podczas transferu sieciowego lub w internecie), będzie to jednoznaczny sygnał, że w organizacji doszło do naruszenia bezpieczeństwa (ang. *security breach*).
## Honeytoken
...
- **Honeytokens** - podobnie jak wspomniane wyżej pliki, celem tej techniki jest przede wszystkim namierzanie naruszeń bezpieczeństwa. Honeytoken to porcja złudnie atrakcyjnych dla atakującego danych (np. fragmenty plików, rekordy w bazie danych, fragmenty konfiguracji, dane uwierzytelniające), które są oczywiście nieprawdziwe. Jednakże systemy bezpieczeństwa organizacji mogą podnieść alarm, gdy te dane zostaną wykryte w sieci lub kiedy ktoś będzie próbował się nimi posłużyć (np. fałszywe dane uwierzytelniające). Będzie to oznaczało, że ktoś nieuprawniony jakoś uzyskał do nich dostęp.
# Materiały źródłowe
