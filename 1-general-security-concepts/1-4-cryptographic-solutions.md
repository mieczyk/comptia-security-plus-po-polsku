...
- Praktyki, których warto się trzymać:
	...
	4. Niektórzy twierdzą, że dobrze jest co jakiś czas zmieniać klucze. Taka strategia może się przydać podczas wycieku klucza - wtedy nieuprawniony posiadacz może uzyskać dostęp tylko do tego wycinka danych, zaszyfrowanych tym kluczem. 
		...
		2. W przypadku kluczy używanych do podpisu (ang. *signing keys*) taka rotacja ma więcej sensu, ponieważ, wtedy minimalizujemy ryzyko podszywania się atakującego i nie musimy kombinować z danymi, które były zaszyfrowane starym kluczem.
	5. Trzymaj kopie zapasowe kluczy prywatnych. Jeśli klucz zostanie utracony (np. wskutek uszkodzenia nośnika), a mamy dostęp tylko do zaszyfrowanych danych, to tak jakbyśmy padli ofiarą *ransomware*. Do bezpiecznego deponowania kluczy można użyć systemów *key escrow*.
	6. Warto używać sprzętowych rozwiązań typu HSM (*Hardware Security Module*), które bezpiecznie przechowują i zarządzają kluczami w taki sposób, że ich użytkowanie staje się dla zwykłego użytkownika transparentne. Prostym przykładem urządzenia opartego o HSM są klucze USB, takie jak np. YubiKey. Wtedy, żeby uzyskać dostęp do chronionych danych, musimy włożyć klucz do portu USB lub przyłożyć do urządzenia mobilnego (wtedy klucz używa technologii NFC do połączenia z telefonem).
## Symmetric
TODO:
